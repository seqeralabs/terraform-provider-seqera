...
resource "seqera_aws_credential" "aws_credential" {
  ...
  # Direct access to AWS keys without discriminated union
  keys = {
...
## Azure Credentials (optional - only created if variables are provided)
resource "seqera_azure_credential" "azure_credential" {
  count        = var.azure_subscription_id != "" ? 1 : 0
  workspace_id = var.workspace_id

  credentials = {
    name          = "test_azure_credential"
    provider_type = "azure"
    description   = "Azure credentials generated by terraform"

    # Direct access to Azure keys without discriminated union
    keys = {
...
## Google Credentials (optional - only created if variables are provided)
resource "seqera_google_credential" "google_credential" {
  count        = var.google_project_id != "" ? 1 : 0
  workspace_id = var.workspace_id

  credentials = {
    name          = "test_google_credential"
    provider_type = "google"
    description   = "Google credentials generated by terraform"

    # Direct access to Google keys without discriminated union
    keys = {
      data = var.google_service_account_key
    }
  }
}
