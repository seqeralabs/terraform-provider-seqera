...
// CredentialsStatus - Validation health for these credentials. AVAILABLE = last probe succeeded (or no probe has been attempted yet). INVALID = the last probe was authoritatively rejected by the provider (see message). Transient probe failures (network / 5xx) leave this field untouched.
...
func (c *CredentialsOutput) GetStatus() *CredentialsStatus {
	if c == nil {
		return nil
	}
	return c.Status
