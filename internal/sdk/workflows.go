...
func (s *Workflows) CreateWorkflowLaunch(ctx context.Context, request operations.CreateWorkflowLaunchRequest, opts ...operations.Option) (*operations.CreateWorkflowLaunchResponse, error) {
	...
	case httpRes.StatusCode == 400:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out operations.CreateWorkflowLaunchResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
...
// UpdateWorkflow - Apply a partial update to the workflow-extension metadata for the given Workflow ID
...
func (s *Workflows) UpdateWorkflow(ctx context.Context, request operations.UpdateWorkflowRequest, opts ...operations.Option) (*operations.UpdateWorkflowResponse, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionRetries,
		operations.SupportedOptionTimeout,
	...
	bodyReader, reqContentType, err := utils.SerializeRequestBody(ctx, request, false, false, "UpdateWorkflowRequest", "json", `request:"mediaType=application/json"`)
	if err != nil {
		return nil, err
	}
...
// SummarizeWorkflowTasks - Summarize the tasks of the given workflow per process, optionally filtered by search
// Rolls up matching tasks per process so the run-timeline grouped view stays complete on large runs, where a capped/paged task list could hide processes whose matches fall past the page. Rejected with a 400 when the run's total task count exceeds 20,000, since the search term does not bound the cost of the underlying query -- only run size does.
func (s *Workflows) SummarizeWorkflowTasks(ctx context.Context, request operations.SummarizeWorkflowTasksRequest, opts ...operations.Option) (*operations.SummarizeWorkflowTasksResponse, error) {
	o := operations.Options{}
	supportedOptions := []string{
		operations.SupportedOptionRetries,
		operations.SupportedOptionTimeout,
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/workflow/{workflowId}/tasks/summary", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	hookCtx := hooks.HookContext{
		...
		BaseURL:          baseURL,
		Context:          ctx,
		OperationID:      "SummarizeWorkflowTasks",
		OAuth2Scopes:     nil,
		SecuritySource:   s.sdkConfiguration.Security,
	}
	...
	res := &operations.SummarizeWorkflowTasksResponse{
		StatusCode:  httpRes.StatusCode,
		ContentType: httpRes.Header.Get("Content-Type"),
		RawResponse: httpRes,
	}
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out shared.ListTaskProcessSummaryResponse
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}

			res.ListTaskProcessSummaryResponse = &out
		default:
			rawBody, err := utils.ConsumeRawBody(httpRes)
			if err != nil {
				return nil, err
